...
class Review extends CI_Model{
	...
	function getAllByBookId($id){
		$query = "SELECT reviews.content, reviews.stars, reviews.user_id, DATE_FORMAT(reviews.created_at, '%M %D, %Y') date, users.alias from reviews 
	...
	function getRecent(){
		$query = "SELECT reviews.content, reviews.stars, DATE_FORMAT(reviews.created_at, '%M %D, %Y %H:%i') date, books.title, books.id as book_id, users.alias, users.id FROM reviews
				JOIN books on reviews.book_id=books.id
				JOIN users on reviews.user_id=users.id
